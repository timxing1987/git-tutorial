...
(function ($, window, document, undefined) {
    ...
    TwbsPagination.prototype = {
        ...
        buildListItems: function (pages) {
            ...
            if (this.options.pagenum) {
                $listItems = $listItems.add(this.buildItem('pagenum', this.options.goPage));
            }

            if (this.options.skip) {
                $listItems = $listItems.add(this.buildItem('skip'), 0);
            }
            /******************修改源码Begin（添加代码片段）***********************/
        ...
        buildItem: function (type, page) {
            ...
            switch (type) {
                ...
                case 'last':
                    ...
                    /******************修改源码Begin（添加代码片段）***********************/
                case 'pagenum':
                    itemText = this.options.pagenum;
                    itemContainer.addClass(this.options.pagenumClass);
                    break;
                case 'skip':
                    itemText = this.options.skip;
                    itemContainer.addClass(this.options.skipClass);
                    break;
                    /******************修改源码Begin（添加代码片段）***********************/
                default:
                    break;
        ...
        setupEvents: function () {
            ...
            this.$listContainer.find('li').each(function () {
                ...
                */
                /************源码End************/
                // 验证
                $this.find('input').bind("keyup", function (evt) {
                    $this.find('input[type=text]').val($this.find('input[type=text]').val().replace(/\D/g, ''));
                    var goindex = $this.find('input[type=text]').val() == '' ? 1 : $this.find('input[type=text]').val();
                    base.options.goPage = (goindex > base.options.totalPages || goindex <= 0) ? $this.find('input[type=text]').val(base.options.currentPage) : goindex;
                    if (event.keyCode == "13") {
                        var pagenum = parseInt(base.options.goPage);
                        pagenum = isNaN(pagenum) ? 1 : pagenum
                        base.show(pagenum);
                    }
                });

                if ($this.attr('class') != base.options.pagenumClass) {
                    $this.click(function (evt) {
                        // Prevent click event if href is not set.
                        !base.options.href && evt.preventDefault();
                        if ($this.attr('class') !== base.options.skipClass) {
                            var pagenum = parseInt($this.data('page'));
                            pagenum = isNaN(pagenum) ? 1 : pagenum
                            base.show(pagenum,10);
                            //base.show(parseInt($this.data('page'), 10));
                        }
                        else {
                            var pagenum = parseInt(base.options.goPage);
    ...
    $.fn.twbsPagination.defaults = {
        ...
        , skip: 'Go',
        skipClass: 'skip',
        pagenum: '<span><input type="text" /></span>',
