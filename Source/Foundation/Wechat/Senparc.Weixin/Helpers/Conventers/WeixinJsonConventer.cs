...
namespace Senparc.Weixin.Helpers
{
    ...
    public class WeixinJsonConventer : JavaScriptConverter
    {
        ...
        {
            _jsonSetting = jsonSetting ?? new JsonSetting();
            _type = type;
        }
        ...
        public override IDictionary<string, object> Serialize(object obj, JavaScriptSerializer serializer)
        {
            ...
            {
                if (!_jsonSetting.PropertiesToIgnore.Contains(propertyInfo.Name))
                {
                    var ignoreProp = propertyInfo.IsDefined(typeof (ScriptIgnoreAttribute), true);

                    if ((_jsonSetting.IgnoreNulls || ignoreProp) && propertyInfo.GetValue(obj, null) == null)
                    {
                        continue;
                    }
