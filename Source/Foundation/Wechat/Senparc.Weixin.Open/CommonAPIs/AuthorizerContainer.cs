...
namespace Senparc.Weixin.Open.CommonAPIs
{
    ...
    public class AuthorizerContainer : BaseContainer<AuthorizerBag>
    {
        ...
        /// <returns></returns>
        public static string TryGetAuthorizerAccessToken(string componentAppId, string authorizerAppid,
        ...
        public static GetAuthorizerInfoResult GetAuthorizerInfoResult(string componentAppId, string authorizerAppid,
            bool getNewTicket = false)
        {
            if (!CheckRegistered(authorizerAppid))
            {
            ...
            lock (authorizerBag.Lock)
            {
                if (getNewTicket || authorizerBag.AuthorizerInfoExpireTime <= DateTime.Now)
                {
                    ...
                    //已过期，重新获取
                    authorizerBag.AuthorizerInfoResult = ComponentApi.GetAuthorizerInfo(componentAccessToken,
                        componentAppId, authorizerAppid); //TODO:如果是过期，可以通过刷新的方式重新获取

                    var componentBag = ComponentContainer.TryGetItem(componentAppId);

                    if (
                        string.IsNullOrEmpty(
                            authorizerBag.AuthorizerInfoResult.authorization_info.authorizer_access_token))
                    {
                        //账号没有此权限
        ...
        /// <param name="getNewTicket">是否强制重新获取新的Ticket</param>
        /// <returns></returns>
        public static JsApiTicketResult GetJsApiTicketResult(string componentAppId, string authorizerAppid,
            bool getNewTicket = false)
        {
            if (!CheckRegistered(authorizerAppid))
            {
            ...
            lock (accessTicketBag.Lock)
            {
                if (getNewTicket || accessTicketBag.JsApiTicketExpireTime <= DateTime.Now)
                {
                    ...
                    accessTicketBag.JsApiTicketExpireTime =
                        DateTime.Now.AddSeconds(accessTicketBag.JsApiTicketResult.expires_in);
                }
            }
