...
namespace Senparc.Weixin.QY.MessageHandlers
{
    ...
    public abstract class QyMessageHandler<TC> : MessageHandler<TC, IRequestMessageBase, IResponseMessageBase>,
        IQyMessageHandler
        where TC : class, IMessageContext<IRequestMessageBase, IResponseMessageBase>, new()
    {
        ...
        /// </summary>
        public static WeixinContext<TC, IRequestMessageBase, IResponseMessageBase> GlobalWeixinContext =
            new WeixinContext<TC, IRequestMessageBase, IResponseMessageBase>();
        ...
        public int AgentId
        {
            get { return EncryptPostData != null ? EncryptPostData.AgentID : -1; }
        }
        ...
        public override XDocument FinalResponseDocument
        {
            get
            {
                ...
                var msgCrype = new WXBizMsgCrypt(_postModel.Token, _postModel.EncodingAESKey, _postModel.CorpId);
                string finalResponseXml = null;
                msgCrype.EncryptMsg(ResponseDocument.ToString(), timeStamp, nonce, ref finalResponseXml);
                    //TODO:这里官方的方法已经把EncryptResponseMessage对应的XML输出出来了
        ...
        {
            get { return base.RequestMessage; }
        ...
        {
            get { return base.ResponseMessage; }
        ...
        public override XDocument Init(XDocument postDataDocument, object postData)
        {
            ...
            var msgCrype = new WXBizMsgCrypt(_postModel.Token, _postModel.EncodingAESKey, _postModel.CorpId);
            string msgXml = null;
            var result = msgCrype.DecryptMsg(_postModel.Msg_Signature, _postModel.Timestamp, _postModel.Nonce,
                postDataStr, ref msgXml);
            /* msgXml
...
<AgentID>2</AgentID>
            ...
            if (OmitRepeatedMessage && CurrentMessageContext.RequestMessages.Count > 1)
            {
                ...
                if ((lastMessage.MsgId != 0 && lastMessage.MsgId == RequestMessage.MsgId) //使用MsgId去重
                    ...
                    ((lastMessage.CreateTime == RequestMessage.CreateTime) &&
                     lastMessage.MsgType != RequestMessage.MsgType) //使用CreateTime去重（OpenId对象已经是同一个）
                    )
                {
        ...
        public virtual IResponseMessageBase OnEventRequest(IRequestMessageEventBase requestMessage)
        {
            ...
            switch (strongRequestMessage.Event)
            {
                ...
                case Event.PIC_PHOTO_OR_ALBUM: //弹出拍照或者相册发图
                    responseMessage =
                        OnEvent_PicPhotoOrAlbumRequest(RequestMessage as RequestMessageEvent_Pic_Photo_Or_Album);
                    break;
                ...
                case Event.SCANCODE_WAITMSG: //扫码推事件且弹出“消息接收中”提示框
                    responseMessage =
                        OnEvent_ScancodeWaitmsgRequest(RequestMessage as RequestMessageEvent_Scancode_Waitmsg);
                    break;
                case Event.LOCATION_SELECT: //弹出地理位置选择器
                    responseMessage =
                        OnEvent_LocationSelectRequest(RequestMessage as RequestMessageEvent_Location_Select);
                    break;
                ...
                case Event.BATCH_JOB_RESULT: //异步任务完成事件推送(batch_job_result)
                    responseMessage =
                        OnEvent_BatchJobResultRequest(RequestMessage as RequestMessageEvent_Batch_Job_Result);
                    break;
                default:
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IResponseMessageBase OnEvent_PicPhotoOrAlbumRequest(
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IResponseMessageBase OnEvent_ScancodeWaitmsgRequest(
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IResponseMessageBase OnEvent_LocationSelectRequest(
        ...
        /// <returns></returns>
        public virtual IResponseMessageBase OnEvent_BatchJobResultRequest(
