    ...
    public static class RequestMessageFactory
    {
        ...
        public static IRequestMessageBase GetRequestEntity(XDocument doc)
        {
            ...
            if (doc.Root.Element("MsgType") != null)
            {
                    ...
                    switch (msgType)
                    {
                        ...
                        default:
                            throw new UnknownRequestMsgTypeException(
                                string.Format("MsgType：{0} 在RequestMessageFactory中没有对应的处理程序！", msgType),
                ...
                {
                    throw new WeixinException(string.Format("RequestMessage转换出错！可能是MsgType不存在！，XML：{0}", doc), ex);
                }
            }
            else if (doc.Root.Element("InfoType") != null)
            {
                ...
                catch (ArgumentException ex)
                {
                    throw new WeixinException(
                        string.Format("RequestMessage转换出错！可能是MsgType和InfoType都不存在！，XML：{0}", doc), ex);
                }
            }
            else
            {
                throw new WeixinException(string.Format("RequestMessage转换出错！可能是MsgType和InfoType都不存在！，XML：{0}", doc));
            }
