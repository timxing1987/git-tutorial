                ...
                if (root.Element(propName) != null)
                {
                    switch (prop.PropertyType.Name)
                    {
                        ...
                        case "List`1": //List<T>类型，ResponseMessageNews适用
                            ...
                            {
                                var mpNewsArticles = new List<MpNewsArticle>();
                        ...
                        case "BatchJobInfo": //异步任务完成事件推送BatchJob
                            var batchJobInfo = new BatchJobInfo();
        ...
        public static XDocument ConvertEntityToXml<T>(this T entity) where T : class, new()
        {
            ...
            */
            ...
            else if (entity is ResponseMessageMpNews)
            {
                propNameOrder.AddRange(new[]
                {
                    "MpNewsArticleCount", "MpNewsArticles", "FuncFlag", /*以下是MpNewsAtricle属性*/ "Title ", "Description ",
                    "PicUrl", "Url"
                });
            }
            ...
            foreach (var prop in props)
            {
                ...
                else
                {
                    switch (prop.PropertyType.Name)
                    {
                        ...
                        case "DateTime":
                            root.Add(new XElement(propName,
                                DateTimeHelper.GetWeixinDateTime((DateTime) prop.GetValue(entity, null))));
                            break;
                        case "Boolean":
                            if (propName == "FuncFlag")
                            {
