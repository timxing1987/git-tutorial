...
namespace Senparc.Weixin.QY.Entities
{
    ...
    public class ResponseMessageBase : MessageBase, IResponseMessageBase
    {
        ...
        public static ResponseMessageBase CreateFromRequestMessage(IRequestMessageBase requestMessage,
                ...
                switch (msgType)
                {
                    ...
                    default:
                        throw new UnknownRequestMsgTypeException(
                            string.Format("ResponseMsgType没有为 {0} 提供对应处理程序。", msgType),
        ...
        public static IResponseMessageBase CreateFromResponseXml(string xml)
        {
            try
            {
                ...
                ResponseMessageBase responseMessage = null;
                var msgType =
                    (ResponseMsgType) Enum.Parse(typeof (ResponseMsgType), doc.Root.Element("MsgType").Value, true);
                switch (msgType)
                {
                    case ResponseMsgType.Text:
                        responseMessage = new ResponseMessageText();
